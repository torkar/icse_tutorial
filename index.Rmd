...
this indicates that the variance needs to be modeled separately. The common way to do this is to use a mixture distribution, i.e., $\mathsf{Negative}\text{-}\mathsf{Binomial}(\lambda, \phi)$, a.k.a. $\mathsf{Gamma}\text{-}\mathsf{Poisson}(\lambda, \phi)$, but we'll leave this for now and take an easier route for pedagogical reasons (if we'd be forced to pick one distribution, then we'd go for $\mathsf{Negative}\text{-}\mathsf{Binomial}(\lambda, \phi)$.
...
# Steps 2--4: Prior checks, sampling, posterior checks
...
What we see here, not surprisingly, is that $\mathcal{M}_3$ is considered the best model, even though we have more parameters in that model (models with more parameters are penalized). However, if we investigate the plot and table a bit closer we see that this is not a clear cut case. The difference between $\mathcal{M}_3$ and $\mathcal{M}_1$ (which is on second place) is $\text{dPSIS} \approx 1.3$ and the standard error is $\text{dSE} \approx 2.4$. Generally speaking, we would like the difference to be at least $2$--$4$ dSE, i.e., dPSIS should preferably be more than, $2.4 \cdot 4 = 9.6$, while in this case it's only $1.3$.

The above tells us at least two things. First, $\mathcal{M}_1$ has, relatively speaking, better out of sample prediction capabilities than $\mathcal{M}_2$, i.e., `TeamExp` has better predictive power than `ManagerExp`, in our models. Second, there is really no significant difference between the four models. A simple general intercept model (i.e., $\mathcal{M}_0$) is almost as good as a model with all predictors (i.e., $\mathcal{M}_3$). Which model should one pick? Well, if,

1. understandability is important, the simplest model should be picked, i.e., $\mathcal{M}_1$ (if there's no large difference between the 'best' and the simplest model),
2. if we want to understand how *all* predictors affect the outcome, then $\mathcal{M}_3$ is the one to pick, and 
3. if out of sample prediction is the only thing we care about then we pick the model that LOO ranks as no. 1, i.e., $\mathcal{M}_3$ again. 

We are explicitly interested in the effect language has on predictions. Let's define our master model to be $\mathcal{M}_3$, i.e., $\mathcal{M} = \mathcal{M}_3$. We could simply create a new model with only a varying intercept according to language. Please do that and compare it with the other models in $\mathbf{M}$.
...
Next, how does the language effect look like when we alter `manager` experience (min and max values), while keeping team experience to it's mean ($\mu = 2.3$ years)? Once again, for Language $3$ the effect is clearly negative, for both low and high experienced managers.
...
Given the above, one conclusion could be that Language $3$ has a fairly strong negative effect on `Effort` compared to the other two languages. In short, Language $3$ requires less `Effort` both when considering `team` and `manager` experience.^[You have the possibility to look at the other predictors which are part of the dataset and include them also in new models to see what happens!]
